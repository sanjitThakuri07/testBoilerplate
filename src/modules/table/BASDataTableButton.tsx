              ...
              <Grid item>
                {tableHeaderContainer && (
                  <Button
                    ...
                    variant="contained"
                    onClick={() => tableHeaderContainer.headerButtonHandler([...selectedData])}
                  >
  ...
  return (
    ...
    >
      {columnName?.toLowerCase() === "risk_factor" ? (
        ...
        >
          <span style={{ background: RadioOptions?.[`${value}`]?.dotColor }}></span>
          {value}
        </div>
...
const BASDataTableButton: React.FC<{
  ...
  const [isHoveringGenerate, setIsHoveringGenerate] = React.useState(false);
  ...
  const handleMouseEnterGenerate = () => {
    setIsHoveringGenerate(true);
  };

  const handleMouseLeaveGenerate = () => {
    setIsHoveringGenerate(false);
  };
  ...
  const deleteHandler = async (datas: object[]) => {
    ...
    let selectedName = datas?.map((data: { name?: string }) => data?.name);
    try {
      ...
      <Box>
        ...
        >
          <Box sx={{ width: "250px", marginTop: "0px" }}>
            <Grid container spacing={0} alignItems="start" justifyContent="start">
              <Grid item xs={12}>
                <Button
                  style={{
                    width: "100%",
                  ...
                  startIcon={<img src="/assets/icons/view.svg" alt="View" />}
                  onMouseEnter={handleMouseEnter}
                  onMouseLeave={handleMouseLeave}
                  endIcon={isHovering ? <img src="/assets/icons/check.svg" /> : <>{"     "}</>}
                >
                  View
                </Button>
              </Grid>
              <Grid item xs={12}>
                <Button
                  ...
                  startIcon={<img src="/assets/icons/bar-chart.svg" alt="Generate" />}
                  onMouseEnter={handleMouseEnterGenerate}
                  onMouseLeave={handleMouseLeaveGenerate}
                  endIcon={
                    isHoveringGenerate ? <img src="/assets/icons/check.svg" /> : <>{"     "}</>
                  }
                >
                  Generate Invoice
                </Button>
                ...
                <Grid item xs={12}>
                  <Button
                    style={{
                      ...
                      padding: "10px",
                      justifyContent: "start",
                      backgroundColor: selectedOption === menu?.label ? "#F2F4F7" : "#ffffff",
                      position: "relative",
                      display: "flex",
                      borderRadius: "0px",
                    }}
              ...
              <TableBody>
                {stableSort(rows as any, getComparator(order, orderBy))
                  ...
                  .map((row: any, index: number) => {
                    let name = row?.[`${tableIndicator?.deleteFieldName}`] || row?.name;
                    ...
                    return (
                      ...
                      >
                            ...
                            return (
                              ...
                              >
                                <Chip
                                  style={{
                                    backgroundColor:
                                      row[hc.id] === "Completed" ? "#c5f7ddf9" : "#a8bff6",
                                    color: row[hc.id] === "Completed" ? "#2e9d63f9" : "#4075f0",
                                  }}
                                  icon={<FiberManualRecordIcon style={{ fontSize: "10px" }} />}
                                  label={row[hc.id] === "Completed" ? "Completed" : "Invoiced"}
                                />
                              </TableCell>
                            );
                          return (
                        ...
                        >
                          <div className="actions-btns-holder" style={{ justifyContent: "start" }}>
                            <Button
                              // onClick={() => {
                              //   let name = row?.[`${tableIndicator?.deleteFieldName}`] || row?.name;
                              //   handleIndividualDelete(row.id, name)
                              // }}
                              startIcon={<img src="/assets/icons/dots-vertical2.svg" alt="More" />}
                            />
                          </div>
                        </TableCell>
                      </TableRow>
...
export default BASDataTableButton;
