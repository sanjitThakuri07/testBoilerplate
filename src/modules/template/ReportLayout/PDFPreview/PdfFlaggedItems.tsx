...
export default function PdfFlaggedItems({ has_flagged_summary, has_flagged }: any) {
  const {
    ...
    <Box mt={2}>
      {has_flagged_summary && (
        <Box className="box-container-pdf">
          ...
          {reducerDatas?.flaggedQuestions?.slice(0, 7)?.map((flag: any, index: any) => {
            return (
              <>
                {index !== 0 && <Divider />}
                <Box key={index} className="individual_box_container">
                  <Stack direction="row" justifyContent="space-between">
                    <Box sx={{ fontSize: "15px" }}>{flag?.label ?? "N/A"}</Box>
                    <Box sx={{ opacity: "0.7", mt: 0.3 }}>
                      {flag?.flaggedResponse?.map((item: any, index: any) => {
                        return <Chip sx={{ ml: 0.5 }} key={index} size="small" label={item} />;
                      })}
                    </Box>
                  </Stack>
                </Box>
