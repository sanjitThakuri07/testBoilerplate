...
export const useTemplateAccessStore = create<any>(
  devtools(
    immer((set) => ({
      templateAccesss: [],
      templateAccess: undefined,
      isLoading: false,
      templateAccesssMedia: [],

      postTemplateAccess: async ({ values, userRoles }: any) => {
        set((state: any) => {
          state.isLoading = true;
        });
        try {
          const response = await postAPI("/templates-access/", values);
          ...
          if (response) {
            set((state: any) => {
              ...
              state.templateAccess = {
                ...state.templateAccess,
                items: [...currentState.templateAccess.items, { ...responseResult?.[0] }],
              };
      ...
      updateTemplateAccess: async ({
        ...
        try {
          const response = await putAPI(`templates-access/${templateAccessId}`, values);
          ...
          if (response) {
            set((state: any) => {
              const currentState = current(state);
              const oldData = currentState.templateAccess.items.find(
                (item: any) => item.id === templateAccessId,
              );
              const indexOfOldData = currentState.templateAccess.items.indexOf(oldData);
              const newData = {
                ...oldData,
                ...responseResult,
              };
              const excludedData = [
                ...currentState.templateAccess.items.filter((item: any) => item !== oldData),
              ];
      ...
      getTemplateAccess: async (templateAccessId: any) => {
        set((state: any) => {
          state.isLoading = true;
        });
        const response = await getAPI(`templates-access/${templateAccessId}`);
        set((state: any) => {
          if (response) {
            state.templateAccess = response.data;
            state.isLoading = false;
          }
        });
      },

      getTemplateAccesss: async ({ query = {} }) => {
        set((state: any) => {
          state.isLoading = true;
        });
        const response = await getAPI(`templates-access/${parseQueryParams(query)}`);
        set((state: any) => {
          if (response) {
            state.templateAccess = response.data;
            state.isLoading = false;
      ...
      deleteTemplateAccess: async (templateAccessIds: any, enqueueSnackbar?: any) => {
        try {
          const response = await deleteAPI("templates-access/", { config_ids: templateAccessIds });
          ...
          set((state: any) => {
            if (response) {
              ...
              state.templateAccess = {
                ...state.templateAccess,
                items: currentState.templateAccess.items.filter(
